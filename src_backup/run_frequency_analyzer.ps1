# PowerShell script to run the Frequency Analyzer for Hill Cipher on Windows
...
# Record start time
$startTime = Get-Date
"Starting frequency analyzer at $startTime" | Out-File -FilePath "$projectRoot\logs\frequency_analyzer.log"

# Display start message
Write-Host "Starting Frequency Analyzer for Hill Cipher at $startTime" -ForegroundColor Green
Write-Host "This will use frequency analysis techniques to break Hill cipher matrices" -ForegroundColor Cyan
Write-Host "- 3x3: Frequency analysis with Portuguese n-grams" -ForegroundColor Cyan
Write-Host "- 4x4: Frequency analysis with Portuguese n-grams" -ForegroundColor Cyan
Write-Host "- 5x5: Frequency analysis with Portuguese n-grams" -ForegroundColor Cyan
...
$topResults = Read-Host "How many top results would you like to see? (Press Enter for default: 10)"
$topParam = ""
if ($topResults -match "^\d+$") {
    $topParam = "--top $topResults"
    Write-Host "Showing top $topResults results" -ForegroundColor Cyan
} else {
    Write-Host "Showing top 10 results (default)" -ForegroundColor Cyan
}

# Run the frequency analyzer
try {
    Write-Host "`nRunning frequency analyzer..." -ForegroundColor Cyan
    
    # Build the command
    $command = "python `"$scriptDir\run_frequency_analyzer.py`" $sizeParam $topParam"
    ...
    $errorMessage = "Error running frequency analyzer: $_"
    Write-Host $errorMessage -ForegroundColor Red
    $errorMessage | Out-File -FilePath "$projectRoot\logs\frequency_analyzer.log" -Append
    $success = $false
}

# Record end time
$endTime = Get-Date
"Processing completed at $endTime" | Out-File -FilePath "$projectRoot\logs\frequency_analyzer.log" -Append

# Calculate and record duration
$duration = $endTime - $startTime
"Total duration: $($duration.Hours) hours, $($duration.Minutes) minutes, $($duration.Seconds) seconds" | Out-File -FilePath "$projectRoot\logs\frequency_analyzer.log" -Append
