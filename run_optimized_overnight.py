#!/usr/bin/env python3
"""
Optimized Overnight Hill Cipher Breaking Script
Uses the optimized CUDA breaker for maximum GPU utilization
...
class OptimizedHillCipherBatchRunner:
    def __init__(self, base_dir=None):
        self.base_dir = Path(base_dir) if base_dir else Path.cwd()
        self.results_dir = self.base_dir / "optimized_overnight_results"
        self.logs_dir = self.results_dir / "logs"
        self.setup_directories()
        self.setup_logging()
        
        # Test cases with optimized batch sizes for GPU utilization
        self.test_cases = [
            {
                "name": "3x3_known",
                ...
                "key_size": 3,
                "max_keys": 10000,
                "batch_size": 4096,  # Larger batch for better GPU utilization
            ...
            },
            {
                ...
                "key_size": 4,
                "max_keys": 100000,
                "batch_size": 2048,  # Balanced for 4x4 matrices
            ...
            },
            {
                ...
                "key_size": 5,
                "max_keys": 1000000,
                "batch_size": 1024,  # Conservative for 5x5 matrices
    ...
    def setup_logging(self):
        ...
        log_file = self.logs_dir / f"optimized_hill_cipher_overnight_{timestamp}.log"
        ...
        self.logger.info("="*80)
        self.logger.info("OPTIMIZED HILL CIPHER OVERNIGHT BREAKING SESSION STARTED")
        self.logger.info("="*80)
    ...
    def run_optimized_cuda_breaker(self, ciphertext, key_size, max_keys, batch_size, test_name):
        """Run the optimized CUDA breaker for a specific test case"""
        ...
        self.logger.info(f"GPU batch size: {batch_size:,}")
        
        # Prepare command
        cuda_breaker_path = self.base_dir / "hill_cipher" / "breakers" / "optimized_cuda_breaker.py"
        cmd = [
            sys.executable,
        ...
        try:
                ...
                for line in lines:
                    if "Best key:" in line:
                        ...
                        try:
                            ...
                            pass
                    elif "Keys per second:" in line:
                        try:
                            keys_per_second = float(line.split("Keys per second:")[1].strip())
                        except:
                            pass
            ...
            if success:
                ...
                if keys_per_second:
                    self.logger.info(f"   GPU Performance: {keys_per_second:.1f} keys/sec")
                if decrypted_text:
    ...
    def save_results(self):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        results_file = self.results_dir / f"optimized_hill_cipher_results_{timestamp}.json"
        ...
        summary = {
            "session_info": {
                ...
                "optimization": "GPU-optimized with large batch processing"
            },
    ...
    def generate_summary(self):
        ...
        gpu_performances = [r.get('keys_per_second', 0) for r in self.results.values() if r.get('keys_per_second')]
        avg_gpu_performance = sum(gpu_performances) / len(gpu_performances) if gpu_performances else 0
        ...
        return {
            ...
            "average_gpu_performance": f"{avg_gpu_performance:.1f} keys/sec",
            "optimization_used": "GPU batch processing with CuPy"
        }
    ...
    def run_all_tests(self):
        ...
        self.logger.info(f"Starting optimized overnight Hill cipher breaking session")
        self.logger.info(f"Total test cases: {len(self.test_cases)}")
        self.logger.info(f"GPU optimization: Large batch processing enabled")
        ...
        for i, test_case in enumerate(self.test_cases, 1):
            ...
            self.logger.info(f"GPU batch size: {test_case['batch_size']:,}")
            self.logger.info("="*60)
            ...
            # Run the test
            result = self.run_optimized_cuda_breaker(
                ciphertext=ciphertext,
        ...
        self.logger.info("="*80)
        self.logger.info("OPTIMIZED OVERNIGHT SESSION COMPLETED")
        self.logger.info("="*80)
        ...
        self.logger.info(f"Average GPU performance: {summary['average_gpu_performance']}")
...
def main():
    """Main function"""
    print("🚀 Optimized Hill Cipher Overnight Breaking Session")
    print("=" * 55)
    print("🔥 GPU-optimized with large batch processing")
    print("⚡ Maximum RTX 4060 Ti utilization")
    
    # Initialize runner
    runner = OptimizedHillCipherBatchRunner()
    
    try:
        # Run all tests
        results = runner.run_all_tests()
        
        print("\n✅ Optimized overnight session completed successfully!")
