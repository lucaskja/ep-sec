...
try:
    ...
    from hill_cipher_breaker_optimized import AdvancedHillBreaker
except ImportError:
    ...
    from src.hill_cipher_breaker_optimized import AdvancedHillBreaker
...
def process_all_ciphers(known_dir: str = "textos_conhecidos", 
    ...
    for size in sizes:
        ...
        if size in [2, 3]:
            ...
            try:
                # Corrigir o erro de argumentos na chamada do método
                advanced_breaker = AdvancedHillBreaker(
                    size, optimized_breaker.language_model, optimized_breaker.dictionary, optimized_breaker.num_threads
                )
                # Modificar a chamada para corresponder à assinatura do método
                results_list = advanced_breaker.break_cipher(ciphertext)
