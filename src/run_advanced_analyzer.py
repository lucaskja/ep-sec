#!/usr/bin/env python3
"""
Script to run the Advanced Frequency Analyzer for Hill Cipher.

This script runs the advanced frequency analyzer on all available ciphertexts.
"""
...
from src.advanced_frequency_analyzer import AdvancedFrequencyAnalyzer
...
def main():
    ...
    # Set up logging
    logging.basicConfig(
        filename="advanced_analyzer.log",
        level=logging.INFO,
...
def process_texts(base_dir: str, sizes: List[int], top_n: int, known: bool = False):
    ...
    for size in sizes:
        ...
        # Run advanced frequency analyzer
        start_time = time.time()
        analyzer = AdvancedFrequencyAnalyzer(size)
        ...
        if results:
            ...
            # Save report
            report_dir = f"relatorios/advanced/{text_type}/hill_{size}x{size}"
...
def generate_report(results: List[Tuple[np.ndarray, str, float]], ciphertext: str, 
    ...
    Returns:
        Report as a string
    """
    report = f"=== Advanced Frequency Analyzer Report ===\n"
    ...
    for i, (matrix, decrypted, score) in enumerate(results):
        ...
        report += f"Common words found: {', '.join(found_common[:20])}\n"
        
        # Add vowel analysis
        vowels = sum(1 for c in decrypted if c in 'AEIOU')
        vowel_ratio = vowels / len(decrypted) if decrypted else 0
        report += f"Vowel ratio: {vowel_ratio:.2f} (ideal for Portuguese: 0.40-0.50)\n"
