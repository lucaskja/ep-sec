#!/usr/bin/env python3
"""
Script to run the frequency-based Hill cipher breaker on all available ciphertexts.
"""
...
# Import our modules
from src.frequency_hill_breaker import break_hill_cipher, load_normalized_text, preprocess_text
from src.hill_cipher import decrypt_hill

def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Frequency-based Hill Cipher Breaker")
    ...
    # Set up logging
    logging.basicConfig(
        filename="frequency_breaker.log",
        level=logging.INFO,
...
def process_texts(base_dir: str, sizes: List[int], normalized_text: str, known: bool = False):
    ...
    for size in sizes:
        ...
        if key_matrix is not None:
            report = f"=== Frequency-based Hill Cipher Breaker Report ===\n"
            ...
            report += f"Common words found: {', '.join(common_words[:10])}\n"
