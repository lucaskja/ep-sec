...
class EnhancedHillBreaker:
    ...
    def generate_statistical_matrices(self, limit: int = 1000) -> List[np.ndarray]:
        ...
        # Import Portuguese statistics for letter frequencies
        try:
            from src.portuguese_statistics import LETTER_FREQUENCIES
            letter_freq = LETTER_FREQUENCIES
        except ImportError:
            try:
                from portuguese_statistics import LETTER_FREQUENCIES
                letter_freq = LETTER_FREQUENCIES
            except ImportError:
                # Fallback to hardcoded frequencies if module not found
                letter_freq = {
        ...
        freq_letters = sorted(letter_freq.items(), 
